...
import { CheckCircle, ArrowRight, Zap, Star, Users } from 'lucide-react';
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-2xl shadow-lg border border-gray-200 p-8">
          ...
          <div className="bg-gradient-to-r from-green-50 to-blue-50 border border-green-200 rounded-xl p-6 mb-8">
            ...
            <p className="text-green-700">
              As a pre-qualified applicant, you get exclusive access to these premium services that can double your funding amount and cut approval time in half.
            </p>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-8 mb-8">
            {offers.map((offer, index) => (
              <motion.div key={offer.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className={`bg-white rounded-2xl shadow-lg border-2 ${offer.popular ? 'border-blue-500 relative' : 'border-gray-200'}`}>
                {offer.popular && <div className="absolute top-4 right-4 bg-blue-500 text-white px-3 py-1 rounded-full text-sm font-medium">Most Popular</div>}
                <div className="p-6">
                  ...
                  <a href={paymentLinks[offer.id]} target="_blank" rel="noopener noreferrer" className={`block w-full text-center font-bold py-3 px-6 rounded-lg transition-colors duration-200 ${offer.popular ? 'bg-blue-600 hover:bg-blue-700 text-white' : 'bg-gray-600 hover:bg-gray-700 text-white'}`}>
                    {offer.cta}
                  </a>
                </div>
              </motion.div>
          ...
          {/* Consultation Call */}
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="bg-gradient-to-r from-purple-500 to-pink-500 rounded-xl p-6 text-center text-white mb-8">
            ...
            <a href={paymentLinks.consultation} target="_blank" rel="noopener noreferrer" className="inline-flex items-center bg-white text-purple-600 font-bold py-3 px-6 rounded-lg hover:bg-gray-100 transition-colors duration-200">
          ...
          {/* Skool Community */}
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="border-2 border-blue-200 bg-blue-50 rounded-xl p-6 text-center">
            <h3 className="text-xl font-bold text-blue-900 mb-3">Join Our Exclusive Community</h3>
            <p className="text-blue-800 mb-4">
              Get access to our private Skool community where you can network with other funded entrepreneurs, get expert advice, and accelerate your business growth.
            </p>
            <a href={skoolCommunityLink} target="_blank" rel="noopener noreferrer" className="inline-flex items-center bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-6 rounded-lg transition-colors duration-200">
          ...
          <div className="text-center mt-6">
            <a href="/thank-you?type=application" className="text-blue-600 hover:text-blue-700 font-medium">
